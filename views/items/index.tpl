...
<head>
    ...
    <title>文档库列表 - Powered by MinDoc</title>
...
{{template "widgets/header.tpl" .}}
    <div class="container manual-body">
        <div class="search-head">
            <strong class="search-title">文档库列表</strong>
        </div>
        <div class="row">
            <div class="hide tag-container-outer" style="border: 0;margin-top: 0;padding: 5px 15px;min-height: 200px;">
                ...
                {{range $index,$item := .Lists}}
                    ...
                    <div class="header">{{$item.ItemName}}</div>
                        <div class="description">书籍数量:{{$item.BookNumber}} &nbsp; 创建人：{{$item.CreateName}}<br/> 创建时间：{{$item.CreateTimeString}}</div>
                    </a>
                {{else}}
                    <div class="search-empty">
                        <img src="{{cdnimg "/static/images/search_empty.png"}}" class="empty-image">
                        <span class="empty-text">没有文档库</span>
                    </div>
                {{end}}
                </div>
            </div>
